...
export default function HomePage() {
  ...
  // ✅ Fix: health sur /api/v1/health + base URL selon environnement
  // ⚠️ Note: si le serveur n'autorise pas X-API-Key en CORS, ce test côté browser peut encore être bloqué.
  const testApiKey = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showMessage('error', `API Key invalide ou endpoint indisponible (${response.status})`);
      }
    } catch (error) {
      console.error('API Key test failed:', error);
      showMessage('error', "Erreur lors du test de l'API Key (CORS ou réseau)");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSelectEndpoint = (endpoint: Endpoint) => {
    ...
    if (isConfigCollapsed) setTimeout(scrollToApiTester, 100);
  };
  ...
  const currentBaseUrl = API_BASE[environment];
                          ...
                          <div>
                            ...
                            <div className="mt-2 text-xs text-gray-600 space-y-1">
                              <p className="flex items-center gap-1">
                                ...
                                <code className="bg-gray-100 px-1.5 py-0.5 rounded">{currentBaseUrl}</code>
                              </p>
